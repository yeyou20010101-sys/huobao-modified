  ...
  <div class="base-select" ref="rootEl">
    ...
    <Teleport to="body">
      <div v-if="isOpen" class="base-select-dropdown" :style="dropdownStyle" ref="dropdownEl">
        ...
        <!-- Options -->
        <div class="base-select-options" ref="optionsEl" :style="optionsStyle">
...
const DROPDOWN_PREFERRED_HEIGHT = 320
...
function positionDropdown() {
  ...
  let openUpward = spaceBelow < DROPDOWN_MIN_HEIGHT && spaceAbove > spaceBelow
  let maxHeight = Math.min(DROPDOWN_PREFERRED_HEIGHT, openUpward ? spaceAbove - DROPDOWN_GAP : spaceBelow - DROPDOWN_GAP)

  if (maxHeight < DROPDOWN_MIN_HEIGHT) {
    openUpward = spaceAbove > spaceBelow
    maxHeight = Math.min(DROPDOWN_PREFERRED_HEIGHT, openUpward ? spaceAbove - DROPDOWN_GAP : spaceBelow - DROPDOWN_GAP)
  }

  const available = openUpward ? spaceAbove : spaceBelow
  maxHeight = Math.max(120, Math.min(maxHeight, available - DROPDOWN_GAP))

  const searchHeight = props.searchable ? SEARCH_BLOCK_HEIGHT : 0
  const optionsMaxHeight = Math.max(72, maxHeight - searchHeight)
  ...
  top = Math.max(
    VIEWPORT_PADDING,
    Math.min(top, window.innerHeight - VIEWPORT_PADDING - maxHeight),
  )
  ...
  optionsStyle.value = {
    maxHeight: `${Math.round(optionsMaxHeight)}px`,
  }
}
