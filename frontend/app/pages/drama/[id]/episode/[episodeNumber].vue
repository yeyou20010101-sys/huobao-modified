...
const gridAssignableShotIds = computed(() => {
  const episodeShotIds = sbs.value.map(s => s.id)
  // 切分分配阶段允许选本集任意镜头，避免被生成时镜头池或旧缓存限制
  if (gridStep.value === 3 || gridStep.value === 4) {
    return episodeShotIds
  }
  let ids
  if (Array.isArray(gridActiveShotIds.value) && gridActiveShotIds.value.length) {
    ids = [...gridActiveShotIds.value]
  } else if (gridMode.value === 'multi_ref') {
    ids = gridSingleTarget.value ? [gridSingleTarget.value] : []
  } else if (gridSelected.value.length) {
    ids = [...gridSelected.value]
  } else {
    ids = episodeShotIds
  }
  return ids.filter(id => episodeShotIds.includes(id))
})
...
function doBreakdown() {
  ...
  runAgent('storyboard_breaker', `请拆解分镜并生成视频提示词。视频模型：${label}。时长规则：优先采用剧本/改写稿中写明的镜头时长（哪怕只有 3 秒），每个镜头的 duration 与 video_prompt 时间段必须一致；仅当剧本未写明时长时，再按叙事需要决定。视频生成时需兼顾该模型能力，但不得擅自把剧本要求的短镜头拉长。`, dramaId, epId.value, refresh)
}
