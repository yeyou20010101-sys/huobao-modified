    ...
    <div v-if="presetDialog" class="overlay" @click.self="presetDialog = false">
      <form class="modal card config-modal" @submit.prevent="applyHuobaoPreset">
        ...
        <div class="huobao-grid">
          <label class="field">
            <span class="field-label">阿里云百炼 API Key <span class="dim">(文本 / Agent)</span></span>
            <input v-model="huobaoForm.aliApiKey" class="input" type="password" placeholder="sk-..." />
            <span class="field-hint"><a href="https://bailian.console.aliyun.com/" target="_blank" rel="noopener">百炼控制台获取 Key →</a></span>
          </label>
          <label class="field">
            <span class="field-label">火山方舟 API Key <span class="dim">(图片 / 视频)</span></span>
            <input v-model="huobaoForm.volcengineApiKey" class="input" type="password" placeholder="ark-..." />
...
const providerPresets = {
  text: {
    ali: { label: '阿里云百炼推荐', baseUrl: 'https://dashscope.aliyuncs.com/compatible-mode/v1', models: ['qwen3.6-plus-2026-04-02'] },
  ...
  image: {
    volcengine: { label: '火山方舟推荐', baseUrl: 'https://ark.cn-beijing.volces.com', models: ['doubao-seedream-5-0-lite-260128'] },
  ...
  video: {
    volcengine: { label: '火山方舟推荐', baseUrl: 'https://ark.cn-beijing.volces.com', models: ['doubao-seedance-2-0-fast-260128'] },
...
async function applyHuobaoPreset() {
  if (!huobaoForm.aliApiKey) {
    toast.warning('请填写阿里云百炼 API Key')
    return
  }
  if (!huobaoForm.volcengineApiKey) {
    toast.warning('请填写火山方舟 API Key')
    return
  }
  try {
    await aiConfigAPI.huobaoPreset({
      ali_api_key: huobaoForm.aliApiKey,
      volcengine_api_key: huobaoForm.volcengineApiKey,
      minimax_api_key: huobaoForm.audioApiKey || undefined,
    })
    await loadCfgs()
    await loadAgents()
...
2. 将剧本拆解为镜头序列；优先采用剧本中写明的镜头时长（哪怕只有 3 秒），未写明时再按叙事需要决定
3. 为每个镜头生成与 duration 一致的视频提示词（video_prompt）
