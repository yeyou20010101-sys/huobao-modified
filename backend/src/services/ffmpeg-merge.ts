...
import { beginTask, refundTaskByRef, settleTaskByRef } from './billing.js'
 ...
 * 拼接一集的所有合成镜头视频
 */
export async function mergeEpisodeVideos(episodeId: number, dramaId: number, userId?: number | null): Promise<number> {
  ...
  const actorId = (typeof userId === 'number' && userId > 0) ? userId : (ownerId ?? 0)
  try {
    beginTask({
      userId: actorId,
      taskType: 'merge',
      ...
      dramaId,
      episodeId,
      refType: 'video_merges',
      refId: mergeId,
      idempotencyKey: `merge:${mergeId}`,
    })
  } catch (err) {
    db.delete(schema.videoMerges).where(eq(schema.videoMerges.id, mergeId)).run()
    throw err
  }

  // 异步执行
  doMerge(mergeId, episodeId, videos, ownerId).catch(err => {
    ...
    refundTaskByRef('video_merges', mergeId, err.message)
  })

  return mergeId
}

async function doMerge(mergeId: number, episodeId: number, videos: string[], userId: number | null) {
  // 生成 concat 列表文件
  const listDir = userStorageAbsDir(userId, 'temp')
  ...
  // 输出文件
  const outputDir = userStorageAbsDir(userId, 'merged')
  ...
  const mergedRelative = toStaticRelative(userId, 'merged', outputFilename)
  ...
  settleTaskByRef('video_merges', mergeId)
}
