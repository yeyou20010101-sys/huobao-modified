...
2. 将剧本拆解为镜头序列，总体保持剧情完整连续
3. 为每个镜头补全完整分镜字段，而不只是 video_prompt
4. 调用 save_storyboards 保存所有分镜

时长规则（重要）：
- 优先服从剧本/原始内容/改写稿里**明确写出的镜头或场景时长**（如「3秒」「5s」「0-3秒」「本镜头8秒」），哪怕只有 3 秒也必须照此设置 duration
- 剧本未写明时长时，再按镜头信息量自行判断，常见范围 3-15 秒，不要为了凑时长而注水
- duration 与 video_prompt 的时间段必须一致；3 秒镜头只写「0-3秒」一段，不要强行拆成多段更长时长
...
- duration：镜头时长（秒，整数，最小 1）。有剧本时长要求时必须遵循；无要求时按叙事需要决定，可短至 3 秒
- scene_id：若可匹配到 scenes 中已有场景，必须填写正确 scene_id

视频提示词格式：
- 按约 3 秒为一段，用时间标记分隔；总跨度必须等于该镜头 duration
