...
function resolveStoryboardDuration(duration?: number | null): number {
    ...
    execute: async ({ storyboards }) => {
      ...
      for (const sb of storyboards) {
        ...
        const res = db.insert(schema.storyboards).values({
          ...
          sceneId: sb.scene_id, duration: resolveStoryboardDuration(sb.duration),
          createdAt: ts, updatedAt: ts,
        }).run()
        syncStoryboardCharacters(Number(res.lastInsertRowid), sb.character_ids || [])
        totalDuration += resolveStoryboardDuration(sb.duration)
      }
